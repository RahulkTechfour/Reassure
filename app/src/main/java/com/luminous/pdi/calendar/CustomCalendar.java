...
import com.luminous.pdi.calendar.adapter.ViewPagerAdapter;
...
public class CustomCalendar extends LinearLayout {
    ...
    public static RecyclerView mRvCalendar;
   ...
   /* public void addAnEvent(String requestDate, int eventCount, ArrayList<EventData> eventDataList) {
        if (!isValidAttr)
            return;

        Event date = new Event();
        date.setDate(requestDate);
        date.setCount(String.valueOf(eventCount));
        date.setEventData(eventDataList);
...
//        int diffYear = endCalendar.get(Calendar.YEAR) - startCalendar.get(Calendar.YEAR); /*if you want to get the 4 year of calender*/
        ...
        mViewPager.setCurrentItem(diffYear); /*for set the calender of the get item*/
     ...
     * @param dateData
     */
    public void setDateSelectionData(List<PendingVisitDatum> dateData) {

        mRvCalendar.setVisibility(View.VISIBLE);
        mTxtEventMessage.setVisibility(View.GONE);
        mTxtEventMessage.setText("");
        ArrayList<PendingVisitDatum> items = new ArrayList<>();
      //  items.clear();
        if (dateData.size() > 0) {
            for (int i = 0; i < dateData.size(); i++) {
                if (dateData.get(i).getDistyDetails() != null && !dateData.get(i).getDistyDetails().isEmpty()) {
                    if (dateData.size() == 0) {
                        ...
                        try {
                            ...
                            // txtSectionHeader.setText(CalendarUtils.getCalendarDateFormat().format(dateTemp));
                            txtSectionHeader.setVisibility(GONE);
             ...
             // change          if (dateData.get(i).getPendingCountDetails().getRequestDate().equalsIgnoreCase(String.valueOf(CurrentDate))) {

                        //   mRvCalendar.setAdapter(new CalendarDataAdapter(dateData.get(i).getDistyDetails()));

                           try {
                               Date dateTemp = CalendarUtils.getCalendarDBFormat().parse(Singleton.getInstance().getCurrentDate());
                               txtSectionHeader.setText(CalendarUtils.getCalendarDateFormat().format(dateTemp));
                               txtSectionHeader.setVisibility(VISIBLE);
